...
with st.sidebar:
    ...
    st.session_state.manual_input = st.toggle("⌨️ Manual Coordinate Entry", 
                                            help="Enable to paste Lat/Lon directly if geocoding is blocked.")
    st.markdown("---")
    if st.session_state.manual_input:
        col_lat, col_lon = st.columns(2)
        h_lat = col_lat.number_input("Hub Lat", value=15.8, format="%.4f")
        h_lon = col_lon.number_input("Hub Lon", value=78.0, format="%.4f")
        if st.button("📍 Add Hub (Manual)"):
            st.session_state.hubs = [{'name': "Manual Hub", 'lat': h_lat, 'lon': h_lon}]
            for v in st.session_state.fleet.values():
                v['pos'] = {'lat': h_lat, 'lon': h_lon}
                v['start_pos'] = {'lat': h_lat, 'lon': h_lon}
                v['status'] = "Ready"
            st.session_state.final_analysis = None
            st.success(f"Manual Hub added at {h_lat}, {h_lon}")
            st.rerun()
    else:
    ...
    if st.session_state.manual_input:
        m_stop_name = st.text_input("Stop Name", value="Manual Stop")
        col_lat_s, col_lon_s = st.columns(2)
        s_lat = col_lat_s.number_input("Stop Lat", value=15.8, format="%.4f", key="s_lat")
        s_lon = col_lon_s.number_input("Stop Lon", value=78.0, format="%.4f", key="s_lon")
        if st.button("📦 Add Stop (Manual)"):
            st.session_state.stops.append({'name': m_stop_name, 'lat': s_lat, 'lon': s_lon})
            st.success(f"Stop added: {m_stop_name}")
            st.rerun()
    else:
